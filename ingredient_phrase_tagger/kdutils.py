...
def import_data(lines):
    ...
    for line in lines:
        ...
        # e.g.: potato \t I2 \t L5 \t NoCAP \t B-NAME/0.978253
            ...
            # print("tag/token/prevName/prevTag", tag, token, prevName, prevTag)

            # print("tag/token/prevTag/prevName", tag, token, prevTag, prevName)
            if tag == "name":
                # print("tag=name")
                if prevName != None and prevTag != "name" and re.search(r"(^|\s)(and|or)$", prevName) == None and re.search(r"^(or|and)(\s|$)", token) == None:
                    # print("prepending comma", token)
                    token = ", " + token
                prevName = token
